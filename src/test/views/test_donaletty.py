...
from odinapi.views.newdonalettyERANC import run_donaletty, Donaletty
...
def ptz_data(inputdatapath, outputdatapath):
    ...
    assert os.path.isfile(
        os.path.join(outputdatapath, "2015", "01", "ZPT_{}.nc".format(scanid))
    )
    return data


@pytest.mark.parametrize(
    "basedir,current_date,hour,expect",
    (
        ("/test1", date(2019, 8, 31), "12", "/test1/2019/08/ei_pl_2019-08-31-12.nc"),
        ("/test2", date(2019, 8, 31), "18", "/test2/2019/08/ei_pl_2019-08-31-18.nc"),
        ("/test3", date(2019, 9, 1), "00", "/test3/2019/09/ea_pl_2019-09-01-00.nc"),
        ("/test4", date(2019, 9, 1), "06", "/test4/2019/09/ea_pl_2019-09-01-06.nc"),
    ),
)
...
def test_loadecmwfdata_reads_expected_files(inputdatapath):
    ...
    assert [donaletty.ecm[0]["fid"].filepath(), donaletty.ecm[1]["fid"].filepath()] == [
...
def test_msis90_values(inputdatapath, parameter, index, expect):
    msis90 = Msis90(solardatafile=os.path.join(inputdatapath, "Solardata2.db"))
    latitude = 11.1
    longitude = 82.0
    ...
    data = {"p": p, "t": t, "z": z}
    assert data[parameter].shape == (76,) and data[parameter][index] == pytest.approx(
        expect, rel=1e-3
    )
