    ...
    def add_project_if_not_exists(self, project_name):
        try:
            self.projects_collection.insert_one(
                {"name": project_name, "development": True}
            )
...
class Level2DB:
    ...
    def _create_indexes(self):
        ...
        self.L2i_collection.create_index([("FreqMode", 1), ("ScanID", 1)], unique=True)
        self.L2i_collection.create_index(
            [("FreqMode", 1), ("ProcessingError", 1), ("Comments", 1), ("ScanID", 1)]
        )
        self.L2_collection.create_index([("FreqMode", 1), ("ScanID", 1)])
        self.L2_collection.create_index(
            [("Product", 1), ("Altitude", 1), ("MJD", 1), ("Location", "2dsphere")]
        )
        self.L2_collection.create_index(
            [
                ("Product", 1),
        ...
        self.L2_collection.create_index(
            [("Product", 1), ("Pressure", 1), ("MJD", 1), ("Location", "2dsphere")]
        )
        self.L2_collection.create_index(
            [
                ("Product", 1),
    ...
    def delete(self, scanid, freqmode):
        ...
        self.L2i_collection.delete_many({"ScanID": scanid, "FreqMode": freqmode})
        self.L2_collection.delete_many({"ScanID": scanid, "FreqMode": freqmode})
    ...
    def get_scan(self, freqmode, scanid):
        ...
        L2i = L2 = L2c = None
        L2i = self.L2i_collection.find_one(match, {"ProcessingError": 0})
        if L2i:
            L2c = L2i.pop("Comments", [])
            L2 = collapse_products(
                list(self.L2_collection.find(match, {"_id": 0, "Location": 0}))
            )
    ...
    def get_L2i(self, freqmode, scanid):
        match = {"ScanID": scanid, "FreqMode": freqmode}
        L2i = self.L2i_collection.find_one(match, {"Comments": 0, "ProcessingError": 0})
        if L2i:
            L2i = replace_id_with_generation_time(L2i)
        return L2i
    ...
    def _get_scans(
        ...
        return self.L2i_collection.find(query, fields).skip(offset).limit(limit)
    ...
    def get_measurements(
        ...
        if areas:
            if isinstance(areas, GeographicArea):
                areas = [areas]
            query = {"$and": [query, {"$or": [area.query for area in areas]}]}
        ...
        for conc in self.L2_collection.find(query, fields, sort=sort, limit=limit):
            yield conc
    ...
    def __init__(self, min_lat=None, max_lat=None, min_lon=None, max_lon=None):
        ...
        if min_lat and max_lat:
            if float(min_lat) > float(max_lat):
                raise ValueError("Min latitude must not be larger than max latitude")
        if min_lon and max_lon:
            if float(min_lon) > float(max_lon):
                raise ValueError("Min longitude must not be larger than max longitude")
...
def expand_product(product):
    ...
    for (
        ...
        vmr,
        avk,
    ) in zip(*[p[array_key] for array_key in PRODUCT_ARRAY_KEYS]):
        doc = {
            "Product": p["Product"],
            "FreqMode": p["FreqMode"],
            "ScanID": p["ScanID"],
...
def get_geojson_point(lat, lon):
    try:
        validate_lat_lon(lat, lon)
        return {"type": "Point", "coordinates": [to_geojson_longitude(lon), lat]}
