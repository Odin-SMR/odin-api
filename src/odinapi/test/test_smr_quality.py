...
class TestQualityControl():
    ...
    def test_check_nr_of_spectra_is_not_ok(
    ...
    def test_check_tb_is_not_ok(self, quality_control):
        ...
        quality_control.check_tb()
        assert (
            quality_control.quality[0:: 5].tolist() ==
            [0, 0x0020])

    def test_check_integration_time_is_ok(
            self, quality_control):
        quality_control.check_int()
        assert np.all(quality_control.quality == 0)

    def test_check_integration_time_is_not_ok(
            self, quality_control):
    ...
    def test_check_observation_sequence_is_ok(
            self, quality_control):
        quality_control.check_obs_sequence()
        assert np.all(quality_control.quality == 0)

    def test_check_observation_sequence_is_not_ok(
            self, quality_control):
        quality_control.refdata['mech_type'][5] = 'SK2'
        quality_control.check_obs_sequence()
        assert (
            quality_control.quality[3: 8].tolist() ==
            [0, 0x0080, 0x0080, 0x0080, 0])

    def test_check_reference_integration_time_is_ok(
            self, quality_control):
        quality_control.check_ref_inttime()
        assert np.all(quality_control.quality == 0)

    def test_check_reference_integration_time_is_not_ok(
            self, quality_control):
        quality_control.refdata['inttime'][5] = 2.87
        quality_control.check_ref_inttime()
        assert (
            quality_control.quality[3: 7].tolist() ==
            [0, 0x0100, 0x0100, 0])

    def test_check_moon_in_mainbeam_is_ok(
            self, quality_control):
        quality_control.check_moon_in_mainbeam()
        assert np.all(quality_control.quality == 0)

    def test_check_moon_in_mainbeam_is_not_ok(
            self, quality_control):
