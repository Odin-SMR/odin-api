...
from odinapi.views.level1b_scandata_exporter_v2 import get_scan_data_v2, plot_scan
from odinapi.views.level1b_scanlogdata_exporter import get_scan_logdata, plot_loginfo
...
class ViewIndex(MethodView):
    ...
    def get(self):
        return render_template("index.html", data=str("ODIN_API_PRODUCTION" in environ))
...
class ViewLevel1(MethodView):
    ...
    def get(self):
        return render_template(
            "level1.html", data=str("ODIN_API_PRODUCTION" in environ)
        )
...
class ViewLevel2PeriodOverview(MethodView):
    ...
    def get(self, project):
        try:
            ...
            params = {
                "start_time": (datetime.today() - timedelta(days=1)).date().isoformat(),
...
class ViewScanSpec(MethodView):
    ...
    def get(self, freqmode, scanno):
        ...
        return send_file(buf, download_name="plot.png", mimetype="image/png")
...
class ViewDataAccess(MethodView):
    ...
    def get(self):
        return render_template(
            "dataaccess.html", data=str("ODIN_API_PRODUCTION" in environ)
        )
