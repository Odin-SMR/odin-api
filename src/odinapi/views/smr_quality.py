...
class QualityControl(object):
    ...
    def check_noise(self):
        ...
        noise_max = 6
        bandwidth = 1e6
        if (self.specdata['efftime'][2] >
                self.specdata['inttime'][2] * 2 or
                self.specdata['efftime'][2] <
                self.specdata['inttime'][2] * 0.5):
    ...
    def estimate_efftime(self, bandwidth, zdiff=10e3):
        ...
        okzind = np.nonzero(self.specdata['altitude'][2::] >=
                            zmax - zdiff)[0]
    ...
    def check_scan(self):
        ...
        test = (np.nonzero((altdiff + 0.1 >= 0))[0].shape[0] ==
                altdiff.shape[0] or
                np.nonzero((altdiff - 0.1 <= 0))[0].shape[0] ==
                altdiff.shape[0])
        if not test:
            self.quality = self.quality + qual
    ...
    def check_ref_inttime(self):
        ...
        for ind, stw in enumerate(self.specdata['stw']):
            ...
            if ind1.shape[0] == 0 or ind2.shape[0] == 0:
                self.quality[ind] = self.quality[ind] + qual
                continue
            if np.abs(self.refdata['inttime'][ind1[-1]] -
    ...
    def check_moon_in_mainbeam(self):
        ...
        qual = 0x0200
        ind1 = np.nonzero(
            (self.specdata['skybeamhit'][2::] & MOONMB == MOONMB)
        )[0]
        if ind1.shape[0] != 0:
    ...
    def filter_references(self):
        ...
        test1 = np.nonzero((self.refdata['sig_type'] == 'REF') &
                           (self.refdata['mech_type'] == 'SK1'))[0]
        test2 = np.nonzero((self.refdata['skybeamhit'] & EARTH1 == EARTH1) |
        ...
        badind2 = np.nonzero((self.refdata['sig_type'] == 'REF') &
                             (self.refdata['mech_type'] != 'SK1'))[0]
