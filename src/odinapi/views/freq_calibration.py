...
class Freqcorr572(object):
    ...
    def get_frequency_grid_in_middle_of_scan(self):
        ...
        around 50 km
        '''
        return self.frequency_grids[
            np.argsort(np.abs(self.altitude - 50e3))[0]
        ]
    ...
    def get_initial_fit_values(self, tbmin=10):
        ...
        if high_tb_index.shape[0] < 2:
            ...
            frequency_initial_guess = self.frequency_grid[
                np.nonzero(self.median_tb == tb_initial_guess)[0]
            ][0] / 1e9
    ...
    def get_tb_profile(self, freq1, freq2):
        ...
        for spectrum in self.spectra:
            if (freq1 > self.frequency_grid[0] / 1e9 and
                ...
                tbi = np.interp(
                    [freq1], self.frequency_grid / 1e9, spectrum
                )
    ...
    def identify_species_by_profile(self,
        ...
        if self.altitude[0] > self.altitude[-1]:
            tb_diff = np.interp(
                [zmin, zmax],
                self.altitude[-1::-1] / 1e3,
                tb_profile[-1::-1, 0]
            )
        else:
            tb_diff = np.interp(
                [zmin, zmax], self.altitude / 1e3, tb_profile[:, 0]
            )
        tbchange = (tb_diff[1] - tb_diff[0]) / 20e3
        return tbchange > cutoff

    def get_initial_fit_values_single_spectrum_correction(
            self, index_of_spectrum):
        ...
        if index_within_frequency_limits.shape[0] > 0:
            ...
            index_greatest_tb = self.spectra[
                index_of_spectrum][index_within_frequency_limits].argmax()
            ...
            tb_initial_guess = self.spectra[
                index_of_spectrum][index_within_frequency_limits].max()
        else:
            frequency_initial_guess = None
            tb_initial_guess = None
    ...
    def get_frequency_offset_of_each_spectrum_in_scan(self):
        ...
        for index_of_spectrum in range(0, self.altitude.shape[0]):
            (frequency_initial_guess, tb_initial_guess) = (
    ...
    def co_is_found_in_fitted_data(
        ...
        if self.altitude_check():
            ...
            tb_profile = self.get_tb_profile(
                frequency_co_line, frequency_o3_line)
    ...
    def get_frequency_offset_of_scan(self):
        ...
        (initial_guess_is_ok, frequency_initial_guess, tb_initial_guess) = (
    ...
    def run_frequency_correction(self):
        (scan_correction_is_ok, frequency_offset_of_scan) = (
            self.get_frequency_offset_of_scan()
        )
        if scan_correction_is_ok:
            self.frequency_grids = (
                self.frequency_grids - (frequency_offset_of_scan * 1e9)
            )
...
def fit_scan_median_spectrum(
    ...
    line_width_o3 = 2e-2
    initial_guess = [
        line_amplitude_co, line_position_co, line_width_co,
        line_amplitude_o3, line_width_o3
    ]
    try:
        popt, _ = curve_fit(
...
def fit_scan_median_spectrum_is_valid(
        frequency_grid, fitted_line_position_co):
    ...
    return (
        ...
        maximum_frequency_difference and
        np.max(frequency_difference[frequency_difference < 0]) >
...
def fit_single_altitude_spectrum(
    ...
    baseline = 0
    initial_guess = [
        line_amplitude, line_position, line_width, baseline
    ]
    try:
        popt, _ = curve_fit(
