...
class Donaletty(dict):
    ...
    def donaletty(self,g5zpt,datetime, newz, lat, lon):
        ...
        msisT=msis.extractPTZprofilevarsolar(datetime, lat,lon, np.arange(75,151,1))[1]
        z=np.r_[g5zpt[g5zpt[:,0]<60,0],np.arange(75,151)]
        temp=np.r_[g5zpt[g5zpt[:,0]<60,2],msisT]
        #newz=np.arange(121)
        normrho=np.interp([20],g5zpt[:,0],g5zpt[:,1])*28.9644/1000/Ro/np.interp([20],
                       g5zpt[:,0],g5zpt[:,2])
    ...
    def loadecmwfdata(self) :
        ...
        self.ecm=[]
        for hourstr in hourlist[ibelow:ibelow+2]:
            ...
            ecmwffilename = self.ecmwfpath+date.strftime('%Y/%m/')+'ODIN_NWP_'+date.strftime('%Y_%m_%d_')+hourstr+'.NC'
            print ecmwffilename
            self.ecm.append(NC.NCecmwf(ecmwffilename))
    ...
    def makeprofile(self, midlat, midlon, datetime, scanid) :
        ...
        ecmz = np.arange(65)*1000 #needs to be in metres 
        newz = np.arange(151)
        ...
        hour = datetime.hour
        ibelow = np.int(np.floor(hour/6))  
        T1 = self.ecm[0].extractprofile_on_z('T',latpt,lonpt,ecmz)
        P1 = self.ecm[0].extractprofile_on_z('P',latpt,lonpt,ecmz)/100. # to hPa
        T2 = self.ecm[1].extractprofile_on_z('T',latpt,lonpt,ecmz)
        P2 = self.ecm[1].extractprofile_on_z('P',latpt,lonpt,ecmz)/100. # to hPa
        ...
        zpt=self.donaletty(np.c_[ecmz/1000,P,T],datetime,newz,midlat,midlon)
        datadict = dict({'ScanID':scanid, 'Z':zpt[:,0],'P':zpt[:,1],'T':zpt[:,2],
                         'latitude':midlat,'longitude':midlon,'datetime':datetime.strftime('%Y-%m-%dT%H:%M:%S')})
        return datadict
...
def save_zptfile(basedir,date,scanid,zpt):
    ...
    rootgrp.description = 'PTZ data for odin-smr level2-processing data source is ECMWF and NRLMSIS91'
