...
class ScanInfoExporter:
    ...
    def extract_scan_log(self, scanid):
        ...
        try:
            scan_data = get_scan_data_v2(self.backend, self.freqmode, scanid)
    ...
    def get_log_of_scans(self, date_start, date_end, scanids):
        ...
        for scanid in scanids:
            ...
            if (
                mjd2datetime(scan_log["MJDStart"]) >= date_start
                and mjd2datetime(scan_log["MJDStart"]) <= date_end
            ):
...
def scan_data_is_valid(scan_data):
    ...
    for item in required_items:
        if item not in scan_data:
            ...
            return False
        if scan_data[item][0] is None or scan_data[item][-1] is None:
...
def get_scan_logdata(backend, datei, freqmode=-1, dmjd=0.25):
    ...
    date_end = date_start + relativedelta(days=+dmjd)  # ty:ignore[invalid-argument-type]
    ...
    scanids = scan_info_exporter.get_scanids(
        mjd2stw(mjd_start - 0.1), mjd2stw(mjd_end + 0.1)
    )
    # loop over scans and extract the desired data
