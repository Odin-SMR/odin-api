...
class TestScanInfoExporter:
    def test_get_scanids(self, scan_info_exporter):
        stw_min = 7014769646
        stw_max = 7014785316
        scanids = scan_info_exporter.get_scanids(stw_min, stw_max)
        assert len(scanids) == 10
        assert scanids[0] == stw_min
        assert scanids[-1] == stw_max

    def test_extract_scan_log(self, scan_info_exporter):
        scan_log = scan_info_exporter.extract_scan_log(7014785316)
        assert scan_log["AltEnd"] == pytest.approx(81062.6, abs=1e-3)
        assert scan_log["AltStart"] == pytest.approx(22603.8, abs=1e-3)
        assert scan_log["DateTime"] == datetime(2015, 1, 12, 0, 17, 12, 315840)
        assert scan_log["FreqMode"] == 2
        assert scan_log["LatEnd"] == pytest.approx(29.42, abs=1e-3)
        assert scan_log["LatStart"] == pytest.approx(25.5651, abs=1e-3)
        assert scan_log["LonEnd"] == pytest.approx(271.474, abs=1e-3)
        assert scan_log["LonStart"] == pytest.approx(271.879, abs=1e-3)
        assert scan_log["MJDEnd"] == pytest.approx(57034.012428, abs=1e-3)
        assert scan_log["MJDStart"] == pytest.approx(57034.0114682, abs=1e-3)
        assert scan_log["NumSpec"] == 21
        assert scan_log["Quality"] == 0
        assert scan_log["ScanID"] == 7014785316
        assert scan_log["SunZD"] == pytest.approx(103.17699, abs=1e-3)
    ...
    def test_get_log_of_scans(self, scan_info_exporter):
        ...
        log_of_scans = scan_info_exporter.get_log_of_scans(
            datetime(2015, 1, 12), datetime(2015, 1, 13), scanids
        )
