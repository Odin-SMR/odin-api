...
def scan_sample_data(db_context):
    ...
    bad_modules = get_bad_ssb_modules(
        scan_data_exporter.spectra["backend"][0], spectra, frequency_grid, False
    )
...
class TestFreqcorr572:
    ...
    def test_get_initial_fit_values(self, freq_corr_572):
        ...
        assert np.all(
            ...
            == [1, pytest.approx(576.5938, abs=1e-3), pytest.approx(82.1877, abs=1e-3)]
        )

    def test_fit_data(self, freq_corr_572):
        freq_corr_572.get_initial_fit_values()
        [fit_is_ok, freq1, freq2, _, _] = fit_scan_median_spectrum(
            576.5938, 82.1877, freq_corr_572.frequency_grid, freq_corr_572.median_tb
        )
        assert np.all(
            [fit_is_ok, freq1, freq2]
            == [
                True,
    ...
    def test_identify_species_by_profile(self, freq_corr_572):
        tb_profile = freq_corr_572.get_tb_profile(576.593, 576.840)
        co_found = freq_corr_572.identify_species_by_profile(tb_profile)
        assert co_found

    def test_identify_species_by_lines(self, freq_corr_572):
        freq_corr_572.get_initial_fit_values()
        [fit_is_ok, _, _, amplitude_1, amplitude_2] = fit_scan_median_spectrum(
            82.1877, 576.5938, freq_corr_572.frequency_grid, freq_corr_572.median_tb
        )
        co_found = identify_species_by_lines(amplitude_1, amplitude_2)
        assert co_found
    ...
    def test_get_frequency_offset_of_scan(
        ...
        assert np.all(
            ...
            == [True, pytest.approx(0.3254, abs=1e-4)]
        )
    ...
    def test_run_frequency_correction(self, freq_corr_572):
        ...
        assert np.max(np.abs(frequencies_offset_per_spectrum)) < 5e-3
